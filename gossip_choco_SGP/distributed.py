...
class GossipDataParallel(Module):
    ...
    def __init__(self, module, device_ids=None, rank=None, world_size=None,
                 ...
                 level=32, biased = False, eta = 0.5, momentum=0.9, lr=0.1, weight_decay = 1e-4, qgm=0, orth_basis=[], inplane_list=[], compress=False, no_layers=5, arch= 'alexnet'):
    ...
    def forward(self, *inputs, **kwargs):
        ...
        #print("inputs inside forward size:", inputs[0].size())
...
#        for s,r in zip(self.module.parameters(), self.neighbor_copy):
#            if self.dist_config['rank']==0: print('send', s[0][0]); break;
#            if self.dist_config['rank']==1: print('recieve', r[0][0]*2.0); break;
    ...
    def _query_gossip_queue(self, non_blocking=False):
        ...
        if self.gossip_flag.is_set():
            ...
            #goss = torch.tensor(goss)
            #print(grad.size())
            #self.dist_config['gossip'] = torch.norm(goss)
    ...
    def transfer_params(self, mix=True, epoch=0, lr=0.1, orth_basis=[], inplane_list=[], compress=False):
        """ Transfers COPY of model parameters to gossip queue """
        self.lr=lr
        self.dist_config['orth_basis']= orth_basis
        self.dist_config['inplane_list']= inplane_list
    ...
    def _gossip_into_receive_buffer(send_buffer, 
        ...
        inp_b= dist_config['inplane_list']
        send_total= []
        ...
        if(dist_config['compress']):
            for i in range(len(send_buffer)):
                if(i in gpm_l):
                    send_en_inp= torch.Tensor([]).to(dist_config['comm_device'])
                    orth_T= torch.t(torch.Tensor(orth_b[int(i/3)]).to(dist_config['comm_device']))
                    if(inp_b[int(i/3)]!=[]):
                        inp_T= torch.t(torch.Tensor(inp_b[int(i/3)]).to(dist_config['comm_device']))
                    '''
                    if(dist_config['rank']==0 and dist_config['epoch']==0):
                        print("inp_b:", np.shape(inp_b[int(i/3)]), i)
                        print("orth_b:", np.shape(orth_b[int(i/3)]),i)
                    '''
                    send_buff_copy= send_buffer[i].clone()
                    send_buff_reshape= send_buff_copy.view(send_buff_copy.size(0), -1) #Cout*(Ci*k*k)   
                    #print("send_buffer_reshape size:", send_buff_reshape.size())
                    send_en= torch.matmul(orth_T, torch.t(send_buff_reshape))
                    if(inp_b[int(i/3)]!=[]):
                    #     if(dist_config['rank']==0):
                    #         if(dist_config['epoch'] in [0, 1e-3, 50, 50+1e-3]):
                    #             print("send_en_imp calculation....",i)
                        send_en_inp= torch.matmul(inp_T, torch.t(send_buff_reshape))

                    send_en_t = torch.cat([send_en, send_en_inp], dim=0).to(dist_config['comm_device'])
                    send_total.append(send_en_t) #encoded
                    sent_layerwise[int(i/3)]= send_en_t.element_size()*send_en_t.nelement()

                    '''
                    rgs= orth_b[int(i/3)]
                    cgs= inp_b[int(i/3)]
                    rgs_s= np.shape(rgs)[1]
                    if(cgs!=[]):
                        cgs_s= np.shape(cgs)[1]

                    decoded= torch.matmul(torch.Tensor(rgs).to(dist_config['comm_device']), send_en_t[0:rgs_s,:])
                    #decoded= torch.matmul(torch.Tensor(orth_b[int(i/3)]).to(dist_config['comm_device']), send_en) #same as O*OT*g
                    if(inp_b[int(i/3)]!=[]):
                        decoded_inp= torch.matmul(torch.Tensor(cgs).to(dist_config['comm_device']), send_en_t[rgs_s:rgs_s+cgs_s,:])
                        #decoded_inp= torch.matmul(torch.Tensor(inp_b[int(i/3)]).to(dist_config['comm_device']), send_en_inp) 
                        decoded_t= decoded+decoded_inp
                    else:
                        decoded_t=decoded
                    
                    if(dist_config['rank']==0):
                        if(dist_config['epoch'] in [0, 1e-3, 50, 50+1e-3] and i in [0,3,6]):
                            norm_orig= torch.norm(torch.t(send_buff_reshape), p=1)
                            norm_dec= torch.norm(decoded_t, p=1)
                            #diff= torch.t(send_buff_reshape) - torch.matmul(torch.matmul(torch.Tensor(orth_b[int(i/3)]).to(dist_config['comm_device']), orth_T), torch.t(send_buff_reshape)) #should be 0 if this is in orth-plane
                            #norm_diff= torch.norm(diff, p=1)

                            print("Mean and var of original:", norm_orig.item()/send_buff_reshape.numel(), torch.var(torch.t(send_buff_reshape)).item(),i, dist_config['epoch'])
                            print("Mean and var of decoded:", decoded_t.size(), norm_dec.item()/decoded_t.numel(), torch.var(decoded_t).item(),i, dist_config['epoch'])
                    '''
            ...
            for i in range(len(send_buffer)):
                if(i in gpm_l):
                    '''
                    if(dist_config['rank']==0 and dist_config['epoch'] in [0, 1e-3, 50, 50+1e-3] and i in [0,3,6]):
                        send_buff_copy= send_buffer[i].clone()
                        send_buff_reshape= send_buff_copy.view(send_buff_copy.size(0), -1) #Cout*(Ci*k*k)   
                        norm_orig= torch.norm(torch.t(send_buff_reshape), p=1)
                        print("Mean and var of original:", norm_orig.item()/send_buff_reshape.numel(), torch.var(torch.t(send_buff_reshape)).item(),i, dist_config['epoch'])
        ...
        # if(dist_config['rank']==0):
        #     if(dist_config['epoch'] in [0, 1e-3, 50, 50+1e-3]):
        #         print("**********Before xi_hat update************")
            # update xi_hat
            '''
            #ADDED FOR DEBUGGING
            for i in range(len(send_buffer)):
                if(i in gpm_l and orth_b!=[] and dist_config['rank']==0):
                    if(dist_config['epoch'] in [0, 1e-3, 1, 1+1e-3, 25, 25+1e-3, 50, 50+1e-3]):
                        orth_T= torch.t((orth_b[int(i/3)]))
                        send_buff_copy= send_buffer[i].clone()
                        send_buff_reshape= send_buff_copy.view(send_buff_copy.size(0), -1) #(Ci*k*k)*Cout

                        encoded= torch.matmul(orth_T, torch.t(send_buff_reshape)) 

                        #diff= torch.t(send_buff_reshape) - torch.matmul(torch.matmul(orth_b[int(i/3)], orth_T), torch.t(send_buff_reshape)) #should be 0 if this is in orth-plane
                        diff= torch.t(send_buff_reshape) - torch.matmul(orth_b[int(i/3)], encoded)
                        print("Mean and var of original:", (torch.norm(torch.t(send_buff_reshape), p=1)).item()/send_buff_reshape.numel(), torch.var(torch.t(send_buff_reshape)).item(), i, dist_config['epoch'])
                        print("Mean and var of error:", (torch.norm(diff, p=1)).item()/diff.numel(), torch.var(diff).item(), i, dist_config['epoch'])
            #ADDED FOR DEBUGGING
            '''
        ...
        if(dist_config['compress']):
            ...
            for r, g in zip(unflatten_tensors(in_msg, send_total), receive_buffer):
                if (i in gpm_l):
                    ...
                    if(dist_config['rank']==0):
                        if(dist_config['epoch'] in [0, 1e-3, 50, 50+1e-3]):
                            print("*******received msg size:", r.size())
                            print("*******rgs matrix shape:", np.shape(rgs))
                            print("*******cgs matrix shape:", np.shape(cgs))
                    '''

                    r_rgs= torch.matmul(torch.Tensor(rgs).to(dist_config['comm_device']), r[0:rgs_s,:])
                    if(inp_b[int(i/3)]!=[]):
                        r_cgs= torch.matmul(torch.Tensor(cgs).to(dist_config['comm_device']), r[rgs_s:rgs_s+cgs_s,:])
                        #r_rgs= torch.matmul(torch.Tensor(orth_b[int(i/3)]).to(dist_config['comm_device']), r)
                        r_decoded= r_rgs+r_cgs
                    else:
                        r_decoded= r_rgs
                    '''
                    if(dist_config['rank']==0):
                        if(dist_config['epoch'] in [0, 1e-3, 50, 50+1e-3] and i in [0,3,6]):
                            norm_recv= torch.norm(r_decoded, p=1)
                            print("Mean and var of recv:", r_decoded.size(), norm_recv.item()/r_decoded.numel(), torch.var(r_decoded).item(),i, dist_config['epoch'])
                    '''
            ...
            for r, g in zip(unflatten_tensors(in_msg, send_buffer), receive_buffer):
                ...
                if(dist_config['rank']==0):
                    if(dist_config['epoch'] in [0, 1e-3, 50, 50+1e-3] and i in [0,3,6]):
                        norm_recv= torch.norm(r, p=1)
                        print("Mean and var of recv:", r.size(), norm_recv.item()/r.numel(), torch.var(r).item(),i, dist_config['epoch'])
                '''
                if dist_config['cpu_comm']:

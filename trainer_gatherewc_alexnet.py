...
parser.add_argument('--arch', '-a', metavar='ARCH', default='alex_quarter', help = 'alexnet or alex_quarter')
...
def run(rank, size, q1, q2):
    ...
    for task_id in range(0, args.num_tasks):
        ...
        for epoch in range(0, args.epochs):  
            ...
            dt, dt_layer, avg_loss= train(train_loader[task_id], model, model_old, criterion, optimizer, epoch, bsz_train, optimizer.param_groups[0]['lr'], device, rank, task_id, c, no_layers, fisher)
            data_per_task += dt
            data_per_task_layer= np.add(data_per_task_layer, dt_layer)

            lr_scheduler.step()
            prec1 = validate(val_loader[task_id], model, model_old, criterion, bsz_val, device, task_id, epoch, c, fisher)
        ...
        if(args.eta!=0.0):
            dt= gossip_avg(train_loader[task_id], model, model_old, criterion, optimizer, epoch, bsz_train, optimizer.param_groups[0]['lr'], device, rank, task_id, c, fisher)
        else:
            print("no gossip averaging in case of turned off communication")
        
        x= validate(val_loader[task_id], model, model_old, criterion, bsz_val, device, task_id, epoch, c, fisher)
        ...
        fisher= fisher_matrix_diag(task_id, train_loader[task_id], model, model_old, criterion, device, c) #fisher calculation at all nodes
        ...
        for tn in range(task_id+1):
            acc_matrix[task_id,jj] = validate(val_loader[tn], model, model_old, criterion, bsz_val, device, tn, epoch, c, fisher) 
...
def fisher_matrix_diag(t, train_loader, model, model_old, criterion, device, c, sbatch=20):
    # Init
    fisher={}
    for n,p in model.named_parameters():
        fisher[n]=0*p.data
...
def train(train_loader, model, model_old, criterion, optimizer, epoch, batch_size, lr, device, rank, task_id, c, no_layers, fisher):
    """
        Run one train epoch
    """
...
def gossip_avg(train_loader, model, model_old, criterion, optimizer, epoch, batch_size, lr, device, rank, task_id, c, fisher):
    """
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss= criterion_ewc(task_id, output[task_id], target_var, criterion, model, model_old, fisher)
        loss.backward()
        optimizer.zero_grad()
        if(task_id==0):
...
def validate(val_loader, model, model_old, criterion, batch_size, device, task_id, epoch, c, fisher):
    """
    Run evaluation
    """
    batch_time = AverageMeter()

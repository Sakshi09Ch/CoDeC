...
###########
from gossip_choco_SGP import GossipDataParallel
from gossip_choco_SGP import RingGraph, GridGraph
from gossip_choco_SGP import UniformMixing
from gossip_choco_SGP import *
from models import *
import xlsxwriter
...
parser.add_argument('--scale_coff', type=int, default=10, metavar='SCF', help='importance co-efficeint (default: 10)')
...
def run(rank, size, q1, q2, q3, q4):
    ...
    if rank==0:
        ...
        print ('-'*40)  
        print("*****SGP calculation at node 0, broadcasted to all other nodes******")
    ...
    for task_id in range(0, args.num_tasks):
        ...
        for epoch in range(0, args.epochs):  
            ...
            dt, dt_layer, avg_loss= train(train_loader[task_id], model, criterion, optimizer, epoch, bsz_train, optimizer.param_groups[0]['lr'], device, rank, feature_mat, task_id, c, no_layers, orth_basis, inplane_list, args.compress)
            data_per_task += dt
        ...
        if(args.eta!=0.0):
            dt= gossip_avg(train_loader[task_id], model, criterion, optimizer, epoch, bsz_train, optimizer.param_groups[0]['lr'], device, rank, task_id, c, orth_basis, inplane_list,args.compress)
        else:
        ...
        if(dist.get_rank()==0):
            ...
            #Ensuring that same GPM feature_list is used by all the nodes/agents

            feature_list, orth_basis, inplane_list, importance_list = update_SGP(mat_list, threshold, orth_basis, feature_list, importance_list, rank=rank, device=device, compress=args.compress)

            for nodes in range(args.world_size-1):
                q1.put(feature_list)
                q2.put(orth_basis)
                q3.put(importance_list)
                q4.put(inplane_list)
        dist.barrier()
...
def train(train_loader, model, criterion, optimizer, epoch, batch_size, lr, device, rank, feature_mat, task_id, c, no_layers, orth_basis, inplane_list, compress):
    """
        Run one train epoch
    """
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if(task_id==0):
            _, amt_data_transfer,amt_data_layerwise = model.transfer_params(epoch=epoch+(1e-3*i), lr=lr, orth_basis=orth_basis, inplane_list= inplane_list, compress=False)
        else:
            _, amt_data_transfer,amt_data_layerwise = model.transfer_params(epoch=epoch+(1e-3*i), lr=lr, orth_basis=orth_basis, inplane_list= inplane_list, compress=compress)            
...
def gossip_avg(train_loader, model, criterion, optimizer, epoch, batch_size, lr, device, rank, task_id, c, orth_basis, inplane_list, compress):
    """
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        if(task_id==0):
            _, amt_data_transfer, _= model.transfer_params(epoch=epoch+(1e-3*i), lr=lr, orth_basis=orth_basis, inplane_list= inplane_list, compress=False)
        else:
            _, amt_data_transfer, _ = model.transfer_params(epoch=epoch+(1e-3*i), lr=lr, orth_basis=orth_basis, inplane_list= inplane_list, compress=compress)
        data_transferred += amt_data_transfer
        if i==n: break
...
def update_SGP (mat_list, threshold, orth_basis=[], feature_list=[], importance_list=[], rank=0, device=None, compress=False):
    inplane_list=[]
    if(rank==0):
    ...
    if not feature_list:
        ...
        for i in range(len(mat_list)):
            ...
            if(compress):
                ...
                for imp in range(r):
                    if(importance_list[i][imp]<1):
                        inp.append(U[:,imp])
                if(inp!=[]):
                    inp= np.array(inp).T.tolist()
                    inplane_list.append(inp)
                else:
                    inplane_list.append([])
        ...
        for i in range(len(mat_list)):
            ...
            if r == 0:
                ...
                if(compress==True):
                    inp=[]
                    f_shape= np.shape(feature_list[i])
                    for imp in range(f_shape[1]):
                        if(importance_list[i][imp]<1):
                            inp.append(feature_list[i][:,imp])
                    if(inp!=[]):
                        inp= np.array(inp).T.tolist()
                        inplane_list.append(inp)
                    else:
    ...
    print('-'*40)
    return feature_list, orth_basis, inplane_list, importance_list
...
def init_process(rank, size, fn, q1, q2, q3, q4, backend=args.backend):
    """Initialize distributed enviornment"""
    os.environ['MASTER_ADDR'] = 'localhost'
    os.environ['MASTER_PORT'] = args.port
    dist.init_process_group(backend, rank=rank, world_size=size)
    fn(rank,size,q1,q2,q3,q4)
...
if __name__ == '__main__':
    ...
    q3= manager.Queue()
    q4= manager.Queue()
    spawn(init_process, args=(size,run,q1,q2,q3,q4), nprocs=size,join=True)

...
class GraphManager(object):
    ...
    def __init__(self, rank, world_size, devices, nprocs_per_node=1, local_rank=0, peers_per_itr=1):
...
class RingGraph_dynamic(GraphManager):
    ...
    def _make_graph(self):
        for rank in range(self.world_size):
            ...
            f5     = [3,4,7,1,6,0,2,5]
            f6     = [5,3,6,0,1,7,4,2]
            self._add_peers(rank, [f_peer, b_peer, f3[rank], f4[rank], f5[rank], f6[rank]])
            #self._add_peers(rank, [f_peer, b_peer])
            #print (rank, f_peer, b_peer, f3, f4)
